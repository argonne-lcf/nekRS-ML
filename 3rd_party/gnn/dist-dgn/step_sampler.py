...
class StepSampler:
    ...
    def sample(
        ...
        assert batch_size <= self.num_diffusion_steps, (
            "Batch size cannot be greater than the number of diffusion steps!"
        )
...
class UniformSampler(StepSampler):
    ...
    @property
    def weights(self) -> torch.Tensor:
        return torch.ones([self.num_diffusion_steps], dtype=self.dtype).to(
            self.device
        )
...
class AdaptiveExponentialSampler(StepSampler):
    ...
    def weights(self) -> torch.Tensor:
        ...
        self.decay_rate = (
            ...
            * (1 + self.decay_rate_increment) ** self.step
        )
