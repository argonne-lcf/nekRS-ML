...
def plot_training_loss(log_file: str):
    ...
    with open(log_file, "r") as f:
        for line in f:
            ...
            mse_loss_match = re.search(
                r"MSE loss term: \[([\d.,\s.e+-]+)\], mean = ([\d.e+-]+)", line
            )
            vlb_loss_match = re.search(
                r"VLB loss term: \[([\d.,\s.e+-]+)\], mean = ([\d.e+-]+)", line
            )
            weighted_mse_match = re.search(
                r"Weighted MSE loss: \[([\d.,\s.e+-]+)\], mean = ([\d.e+-]+)",
                line,
            )
            diffusion_steps_match = re.search(
                r"Sampled diffusion steps: \[([\d,\s]+)\]", line
            )
            ...
            if mse_loss_match:
                ...
                loss_values = [
                    float(x.strip()) for x in loss_list_str.split(",")
                ]
            ...
            if vlb_loss_match:
                ...
                loss_values = [
                    abs(float(x.strip())) for x in loss_list_str.split(",")
                ]
                for step, loss_val in zip(steps, loss_values):
    ...
    has_weighted = any(
        len(v) > 0 for v in per_step_weighted_mse_losses.values()
    )
    ...
    if has_weighted:
        w_steps = sorted([
            s
            for s in per_step_weighted_mse_losses.keys()
            if len(per_step_weighted_mse_losses[s]) > 0
        ])
        w_steps_to_plot = w_steps[::10]
        for step in w_steps_to_plot:
            plt.plot(
                range(len(per_step_weighted_mse_losses[step])),
                per_step_weighted_mse_losses[step],
                label=f"{step}",
                linewidth=1,
            )
        ...
        mse_steps = sorted([
            s
            for s in per_step_mse_losses.keys()
            if len(per_step_mse_losses[s]) > 0
        ])
        mse_steps_to_plot = mse_steps[::10]
        for step in mse_steps_to_plot:
            plt.plot(
                range(len(per_step_mse_losses[step])),
                per_step_mse_losses[step],
                label=f"{step}",
                linewidth=1,
            )
        plt.ylabel("MSE Loss")
    ...
    plt.subplot(1, 2, 2)
    vlb_steps = sorted([
        s for s in per_step_vlb_losses.keys() if len(per_step_vlb_losses[s]) > 0
    ])
    vlb_steps_to_plot = vlb_steps[::10]
    for step in vlb_steps_to_plot:
        plt.plot(
            range(len(per_step_vlb_losses[step])),
            per_step_vlb_losses[step],
            label=f"{step}",
            linewidth=1,
        )
    plt.xlabel("Instances in training")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--log_file", type=str, default="train.log", help="Log file to plot"
    )
