...
class MLPReprod:
    ...
    def train(self, graph_component: Optional[str] = "nodes"):
        ...
        step_flops = (
            3 * mlp_flops
        )  # backward pass is approx. 2x forward pass flops
        sizeof_dtype = torch.tensor(
            [], dtype=self.torch_dtype
        ).element_size()  # in bytes
        gemm_mem_traffic = sizeof_dtype * (
        ...
        log.info(
            f"\tFLOPs per sec: {step_flops / compute_time / 1e12:.4f} TFLOPS"
        )
        log.info(f"\tMem traffic: {step_mem_traffic / 1e9:.4f} GB")
        log.info(
            f"\tEffective Mem BW: {step_mem_traffic / 1e9 / compute_time:.4f} GB/s"
        )
        log.info("\n")
