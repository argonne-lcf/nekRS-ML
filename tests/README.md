...
Options:
  ...
  --dry-run,     -d                    Generate the job scripts (rfm_job.sh) without submitting them
  --nekrs-home,  -nh  <DIR>            Existing nekRS install to satisfy the build fixtures under --dry-run
                                       (Default: the examples/ tree of this repository)
...
Examples:
  ...
  You can pass --list-tags or -l to list all the test tags:
    ./run.sh -l
  To inspect the job script a test would submit, without running anything:
    ./run.sh -t turbchannel_srgnn$ -d
  Note that -d and -b are mutually exclusive: ReFrame's dry run skips the
  compile phase, so the build fixtures never produce an install. Instead, -d
  stubs out their stage directories (see --nekrs-home).
```
...
## Inspecting a Test's Job Script

Each test assembles its commands in ReFrame pre-run hooks, so what actually runs is
only visible in the `rfm_job.sh` that ReFrame generates. To see that script without
submitting anything, pass `--dry-run`/`-d`:

```sh
./run.sh -t turbchannel_srgnn$ -d
```

This prints the path of every generated `rfm_job.sh` at the end. The scripts live in
`${PREFIX}/stage/<system>/<partition>/<PrgEnv>/<test>/rfm_job.sh`. Since `-d` omits
`--timestamp`, that path stays the same across repeated dry runs, which makes it
convenient to diff after editing a test.

Note that `-d` cannot be combined with `-b`. ReFrame's dry run skips the compile
phase, so the `NekRSBuild` and `SmartRedisBuild` fixtures never produce an install
tree, and the tests would fail while reading real files from it (for example,
`NekRSMLTest.order()` greps `<case>.par` for the polynomial orders). Instead, `-d`
pre-creates the fixtures' stage directories and points the nekRS install's
`examples/` at this repository's own `examples/` tree. If you want to use a
different install, pass `--nekrs-home/-nh <DIR>`:

```sh
./run.sh -t turbchannel_srgnn$ -d -nh /path/to/nekRS/install
```

Everything else the tests reference from the install is only interpolated into
command strings, so it does not need to exist for a dry run. The generated script is
therefore accurate about the commands and their ordering, but the paths it contains
point into the stubbed stage directory rather than a real build.
