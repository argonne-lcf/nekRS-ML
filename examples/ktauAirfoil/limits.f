C-----------------------------------------------------------------------
      subroutine get_limits(phi,phimin,phimax,phiave,
     &                      dphi,phip,rmsphi,n,dtime)
      implicit none
      include 'SIZE'
      include 'TOTAL'
      ...
      if(istep.ge.1) then
        ...
        rmsphi=sqrt(rmsphi/real(ntot))/dtime
      endif
      dphi=glmax(dphi,1)
      dphi=dphi/dtime
...
c-----------------------------------------------------------------------
      ...
      do e=1,nelv
        ...
        enddo
      enddo
c..   for elements with edges but not faces along a wall
...
C
      ...
      integer ntot
      real msk(lx1,ly1,lz1,lelv),ypout(lx1*ly1*lz1*lelv)
...
C       build the mask  (this mask ignores some points which maybe
C       important... 
        call rone(msk,nx1*ny1*nz1*nelv)    ! need to look at it more
closely)
        call rzero(ypout,nx1*ny1*nz1*nelv) ! dump yp info for debugging
C        call copy(t(1,1,1,1,1),msk(1,1,1,1),ntot)
        do e=1,nelv
          do isd=1,2*ndim
            if(cbc(isd,e,1).eq.'W  ') then
              call backpts(i0,i1,j0,j1,k0,k1,isd)
...
C     Primitive Variables
      ...
      if(ifflow) then
        call get_limits(vx,uxmin,uxmax,uxave,dux,vxlag,rmsux,n1,dtime)
        call get_limits(vy,uymin,uymax,uyave,duy,vylag,rmsuy,n1,dtime)
       if(if3d) call 
     &       get_limits(vz,uzmin,uzmax,uzave,duz,vzlag,rmsuz,n1,dtime)
        call get_limits(pr,prmin,prmax,prave,dpr,prlag,rmspr,n2,dtime)
      endif
      if(ifheat) then
        do i=1,npscal+1
          nf=n1
          if(iftmsh(i+1))nf=nt
          call get_limits(t(1,1,1,1,i),thmin(i),thmax(i),thave(i),
     &                    dth(i),tlag(1,1,1,1,1,i),rmsth(i),nf,dtime)
        enddo
      endif
