...
class ShootingWorkflow:
    ...
    def __init__(self, cfg: DictConfig) -> None:
        ...
        self.exp = Experiment(
            self.cfg.database.exp_name, launcher=self.cfg.database.launcher
        )
    ...
    def assignNodes(self) -> None:
        ...
        if self.cfg.database.deployment == "clustered":
            ...
            self.db_nodes = ",".join(
                self.nodelist[
                    self.cfg.run_args.sim_nodes : self.cfg.run_args.sim_nodes
                    + self.cfg.run_args.db_nodes
                ]
            )
            self.dbNodes_list = self.nodelist[
                self.cfg.run_args.sim_nodes : self.cfg.run_args.sim_nodes
                + self.cfg.run_args.db_nodes
            ]
            self.train_nodes = ",".join(
                self.nodelist[
                    self.cfg.run_args.sim_nodes
                    + self.cfg.run_args.db_nodes : self.cfg.run_args.sim_nodes
                    + self.cfg.run_args.db_nodes
            ...
            print(
                f"Database, nekRS, training and inference running on {self.cfg.run_args.sim_nodes} nodes:"
            )
            print(self.sim_nodes, "\n", flush=True)
    ...
    def launchClusteredDB(self) -> None:
        ...
        if self.cfg.database.launcher == "pals":
            run_command = "mpiexec"
        network = (
            self.cfg.database.network_interface
            if type(self.cfg.database.network_interface) == str
            else OmegaConf.to_object(self.cfg.database.network_interface)
        )
        self.db = self.exp.create_database(
            port=self.port,
            batch=False,
    ...
    def launchPersistentCoDB(self) -> None:
        ...
        coDB_manager_settings = PalsMpiexecSettings(
            "python", exe_args=fname, run_args=None, env_vars=None
        )
        ...
        db_bind = (
            None
            if self.cfg.run_args.db_cpu_bind == "None"
            else self.cfg.run_args.db_cpu_bind
        )
        db_bind_cores = 0 if db_bind is None else len(db_bind)
        coDB_manager_settings.set_cpu_binding_type(
            f"list:{psutil.cpu_count(logical=False) - db_bind_cores - 1}"
        )
    ...
    def stopPersistentCoDB(self) -> None:
        ...
        env_vars = {"SSDB": SSDB}
        stop_settings = PalsMpiexecSettings(
            "python", exe_args=fname, run_args=None, env_vars=env_vars
        )
    ...
    def launchNekRS(self) -> None:
        ...
        nrs_settings = PalsMpiexecSettings(
            client_exe, exe_args=None, run_args=None, env_vars=env_vars
        )
        ...
        if self.cfg.sim.affinity:
            nrs_settings.set_gpu_affinity_script(
                self.cfg.sim.affinity, self.cfg.run_args.simprocs_pn
            )

        print("Launching nekRS ...")
        self.nekrs_model = self.exp.create_model(
            f"nekrs_{self.fine_tune_iter}", nrs_settings
        )
    ...
    def launchTrainer(self) -> None:
        ...
        if self.cfg.train.affinity:
            skip = (
                0
                if self.cfg.database.deployment == "clustered"
                else self.cfg.run_args.simprocs_pn
            )
            ml_settings.set_gpu_affinity_script(
                self.cfg.train.affinity, self.cfg.run_args.mlprocs_pn, skip
            )

        print("Launching GNN training ... ")
        self.train_model = self.exp.create_model(
            f"train_{self.fine_tune_iter}", ml_settings
        )
        if len(self.cfg.train.copy_files) > 0 or len(self.cfg.train.link_files) > 0:
            self.train_model.attach_generator_files(
                to_copy=list(self.cfg.train.copy_files),
    ...
    def launchInference(self) -> None:
        ...
        ml_exe = (
            ml_exe + " " + self.cfg.inference.arguments + f" master_addr={self.host}"
        )
        ml_exe = (
            ml_exe
            + f" model_dir={os.getcwd()}/nekRS-ML/train_{self.fine_tune_iter}/saved_models/"
        )
        ml_settings = PalsMpiexecSettings(
            "python",
            exe_args=ml_exe,
        ...
        self.infer_model = self.exp.create_model(
            f"infer_{self.inference_iter}", ml_settings
        )
        if (
            len(self.cfg.inference.copy_files) > 0
            or len(self.cfg.inference.link_files) > 0
        ):
            self.infer_model.attach_generator_files(
                to_copy=list(self.cfg.inference.copy_files),
        ...
        with open(f"{self.log_dir}/nekrs_0/nekrs_0.out", "r") as fh:
            for l in fh:
                ...
                if " udfExecuteStep " in l:
                    udf_time = float(
                        l.split("udfExecuteStep")[-1].split("s")[0].strip()
                    )
