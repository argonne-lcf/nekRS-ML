...
static deviceMemory<dfloat> o_tmp1, o_tmp2;
...
void writeStatistics(nrs_t *nrs, dfloat time, int tstep, const std::string& fname)
{
  ...
  //static deviceMemory<dfloat> o_fields1D;
...
void calc_wallModel(dfloat *usrwrk, dlong fieldOffset, int tstep, int Npart, std::vector<dlong> BdryToV, std::vector<dfloat> Upart)
{
...
void setupRecycledInflow(nrs_t *nrs)
{
  ...
  poolDeviceMemory<dlong> o_BdryToV(npts);
  poolDeviceMemory<dfloat> o_Upart(nrs->NVfields * npts);
  //deviceMemory<dfloat> o_xpart = platform->device.malloc(npts, sizeof(dfloat));
  //deviceMemory<dfloat> o_ypart = platform->device.malloc(npts, sizeof(dfloat));
  //deviceMemory<dfloat> o_zpart = platform->device.malloc(npts, sizeof(dfloat));
  ...
  o_tmp1 = poolDeviceMemory<dfloat>(Nblock);
  o_tmp2 = poolDeviceMemory<dfloat>(Nblock);
...
void UDF_Setup()
{
  ...
  if(caseType == "WMLES"){
    usrwrk = (dfloat *) calloc(nrs->fieldOffset, sizeof(dfloat));
    ...
    std::vector<dlong> BdryToV(npts);
    std::vector<dfloat> Upart(nrs->NVfields * npts);
...
void UDF_ExecuteStep(dfloat time, int tstep)
{
  ...
  //deviceMemory<dfloat> o_Vx = nrs->o_U.slice(0 * fieldOffsetBytes, fieldOffsetBytes);
  //deviceMemory<dfloat> o_Vy = nrs->o_U.slice(1 * fieldOffsetBytes, fieldOffsetBytes);
  //deviceMemory<dfloat> o_Vz = nrs->o_U.slice(2 * fieldOffsetBytes, fieldOffsetBytes);
  //deviceMemory<dfloat> o_Pr = nrs->o_P;
  ...
  if(caseType == "WMLES"){
    if (tstep>0) {
      ...
      nrs->o_usrwrk.copyFrom(usrwrk, nrs->fieldOffset * sizeof(dfloat));
    }
  }
