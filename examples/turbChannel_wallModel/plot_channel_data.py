...
print(
    "--||SOD :: Mesh size : nx[{}] ny[{}] nz[{}]".format(
        len(xlin), len(ylin), len(zlin)
    )
)
...
ubulkDNS = np.trapz(utauDNS * dns[:, 2], dns[:, 0]) / (
    np.amax(dns[:, 0]) - np.amin(dns[:, 0])
)
