...
for i in range(pos_size1.shape[0]):
    ...
    for j in range(pos_rank0_size2.shape[0]):
        if np.allclose(pos_size1[i], pos_rank0_size2[j]):
            print(f"Node {i}:{pos_size1[i]} in the size 1 case is the same as node {j}:{pos_rank0_size2[j]} in the rank 0 size 2 case", flush=True)
            is_rank0_found = True
            break
    for k in range(pos_rank1_size2.shape[0]):
        if np.allclose(pos_size1[i], pos_rank1_size2[k]):
            print(f"Node {i}:{pos_size1[i]} in the size 1 case is the same as node {k}:{pos_rank1_size2[k]} in the rank 1 size 2 case", flush=True)
            is_rank1_found = True
            break
    ...
    if is_rank0_found and is_rank1_found:
        ...
        if not np.allclose(errors_rank0_size2[j] + errors_rank1_size2[k], errors_size1[i]):
            print(f"Node {i}:{pos_size1[i]} has error {errors_size1[i]} on size 1 case, but {errors_rank0_size2[j]} on rank 0 case and {errors_rank1_size2[k]} on rank 1 case on size 2 case", flush=True)
    #elif is_rank0_found:
