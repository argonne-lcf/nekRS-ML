...
def train(cfg: DictConfig, client: Optional[OnlineClient] = None) -> None:
    ...
    while True:
        ...
        if RANK == 0:
            print(
                f"[Trainer] Loaded data for step {iteration} in {stream_time:.4f} seconds",
                flush=True,
            )
        ...
        if not np.allclose(inputs, outputs) and not np.allclose(
            inputs, np.ones_like(inputs) * (rank + frac)
        ):
            print(
                f"[ML] Input and output are not correct on rank {RANK} for step {iteration}",
                flush=True,
            )
            MPI.Abort(COMM, 1)
        COMM.Barrier()
    ...
    if cfg.online:
        if RANK == 0:
            print(f"[Trainer] Telling simulation to quit ...", flush=True)
        client.stop_nekRS()
    COMM.Barrier()
    ...
    if RANK == 0:
        ...
        recv_bw = (
            2 * data_size_gb / avg_stream_times
        )  # 2x because stream time is for both Uin and Uout
...
def main(cfg: DictConfig) -> None:
    if RANK == 0:
        print(
            f"[Trainer] Running with {SIZE} MPI ranks on head node {HOST_NAME}",
            flush=True,
        )
    ...
    if not cfg.online:
        ...
        COMM.Barrier()
        if RANK == 0:
            print("[Trainer] Initialized Online Client!", flush=True)
        train(cfg, client)
