...
class NekRSKershawCase(NekRSCase):
    ...
    def __init__(self, case_number):
        super().__init__(
            name=self.case_name,
            directory=f"{self.case_root}/{self.case_name}_{case_number:04}",
        )
...
class NekRSKershawTest(NekRSTest):
    ...
    def validate_case_number(self):
        if self.case_number not in self.valid_case_numbers:
            self.skip(
                f'kershaw_{self.case_number} not valid a valid case. "case_number" be one of {self.valid_case_numbers}.'
            )
    ...
    def check_constraints(self):
        ...
        self.skip_if(
            ...
            f"This test requires at least {min_nodes} nodes.",
        )
        self.skip_if(
            self.num_nodes > max_nodes,
            f"This should run on at most {max_nodes} nodes.",
        )
