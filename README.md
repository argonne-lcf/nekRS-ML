...
* Graph-based data-driven modeling: 
  ...
  * [Dist-GT](./3rd_party/gnn/dist-gnn/), where GT stands for graph transformer, is a variant of the Dist-GNN model which uses transformer layers instead of traditional message passing layers inside the processor. The model performs element-wise local attention, meaning attention is computed only among the GLL points, considered tokens by the transformer, belonging to the same spectral element. Consistency is still enforced for this model by following each local attention update with a reconciliation step to enforce that nodes on element boundaries corresponding to the same physical location have an identical feature representation. This is done for both co-incident nodes within a rank and across ranks, thus ensuring consistency for partitioned graphs. When using multiple successive transformer layers, the reconciliation step is also responsible for propagating information across elements of the mesh. For detailed information of the Dist-GT model, please see the following [paper](https://ml4physicalsciences.github.io/2025/files/NeurIPS_ML4PS_2025_257.pdf).
